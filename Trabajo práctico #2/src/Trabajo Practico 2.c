...
int main(void) {
    ...
    if(!respuesta){
    	...
    	case 1 :
    		...
    		if(addEmployee(arrayEmpleados,CANT_EMPLEADOS,&id,arrayEmpleados->name,arrayEmpleados->lastName,arrayEmpleados->salary,arrayEmpleados->sector) == 0)
    		{
    			flag++;
    	...
    	case 2 :
    		if(flag != 0){
    			idModify = findEmployeeById(arrayEmpleados,CANT_EMPLEADOS,id);

    			if(idModify != -1){

    		modifyEmployeeById(arrayEmpleados,CANT_EMPLEADOS,idModify);
    	...
    	case 3 :
    		...
    		idRemove = findEmployeeById(arrayEmpleados,CANT_EMPLEADOS,id);

    		if(removeEmployee(arrayEmpleados,CANT_EMPLEADOS,idRemove)!= -1){
    	...
    	case 4 :
    		if(flag != 0){
  	    	...
  	    	utn_calcularPromediosueldo(arrayEmpleados,CANT_EMPLEADOS);
    		salida = 0;
  	   	                  };
    	 }else{
    		  	printf("\nPrimero debe cargar un empleado\n");
