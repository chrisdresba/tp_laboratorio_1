...
int addEmployee(Employees* list, int limite, int* id, char name[],char lastName[],float salary,int sector)
{
	...
	utn_getNumerofloat(&buffer.salary,"\n Ingrese el salario: ","\nIngreso un dato incorrecto: 0-500000",0,500000,2) == 0 &&
...
if(list != NULL && limite > 0){
	do{
		...
		if(order == 0){
			if(strcmp(list[i].lastName,list[i+1].lastName) >0 || (strcmp(list[i].lastName,list[i+1].lastName) == 0 && list[i].sector < list[i+1].sector)){
		...
		}else if(strcmp(list[i].lastName,list[i+1].lastName) < 0 || (strcmp(list[i].lastName,list[i+1].lastName) == 0 && list[i].sector > list[i+1].sector)){
...
int printEmployees(Employees* list, int limite)
{
	...
	if(list != NULL && limite > 0){
		...
		for(i=0; i <limite;i++){
			...
			printf("\nID %d - Nombre: %s, %s - Sueldo: $%.2f - Sector: %d.",list[i].id,list[i].lastName,list[i].name,list[i].salary,list[i].sector);
		}
	  };
		retorno=0;
	}
