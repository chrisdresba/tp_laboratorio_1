...
int utn_getNombre(char* pResultado, int longitud,char* mensaje, char* mensajeError, int reintentos)
{
	...
	if(pResultado != NULL && mensaje != NULL  && longitud > 0 && mensajeError != NULL && reintentos >=0){
	...
	while(reintentos >=0)
		...
		for(i=0 ; bufferString[i] != '\0' && i < longitud; i++)
				{
			if((!(isspace(bufferString[i]))) && (!(isalpha(bufferString[i])))) {
...
void utn_calcularPromediosueldo(Employees* list, int limite){
	...
	printf("\nEl total es $%.2f, con un sueldo promedio de $%.2f. \nLa cantidad de empleados que superan el sueldo promedio es %d persona/s",acum,pResultado,pCant);
	}
